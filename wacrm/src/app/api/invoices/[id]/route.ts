...
const updateSchema = z.object({
  ...
  status: z.enum(['DRAFT', 'SENT', 'RECEIVED', 'PAID', 'OVERDUE', 'CANCELLED']).optional(),
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Can only delete unpaid invoices (DRAFT, SENT, RECEIVED, UNPAID, OVERDUE)
    const deletableStatuses = ['DRAFT', 'SENT', 'RECEIVED', 'UNPAID', 'OVERDUE'];
    if (!deletableStatuses.includes(invoice.status)) {
      return NextResponse.json({ error: `Cannot delete ${invoice.status} invoices` }, { status: 400 });
    }
